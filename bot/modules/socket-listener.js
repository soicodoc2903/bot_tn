...
import {getRandom, haiLoc, moLiXi, checkEvent, bacQuyEvent, napQuyEvent} from "./event.js";
...
export async function xuLyChat(content = '', tcvId = 0, cboxId = '', tcvName = '') {
    ...
    if (!content.startsWith(".")) {
        ...
        if (text.toLowerCase().startsWith('hái lộc')) {
            axios.get(`https://soi-tcvtool.xyz/truyencv/member/${tcvId}`).then(
                response => { 
                    ...
                    if (bp !== '๖ۣۜTiếu ๖ۣۜNgạo ๖ۣۜNhân ๖ۣۜGian') {
        ...
        if (text.toLowerCase().startsWith('check dtl')) {
            await showDangTienLenh(tcvId, cboxId);
            return 1;
        }
        ...
        if (msg.toLowerCase().includes('hi mèo') || msg.toLowerCase().includes('mèo ơi') || msg.toLowerCase().startsWith('mèo')) {
            const basic = await getUserInfo(tcvId);
        ...
        if (text.toLowerCase().startsWith('list acc đan') && tcvId == 132301) {
            await getListAcc();
            return 1;
        }

        if (text.toLowerCase().startsWith('check log đan') && tcvId == 132301) {
            await getLogDan();
            return 1;
        }

        /*if (text.toLowerCase().startsWith('bật luyện đan cho id')) {
            const toId = args[5];
            await activeDan(tcvName, toId);
            return 1;
        }

        if (text.toLowerCase().startsWith('tắt luyện đan cho id')) {
            const toId = args[5];
            await inActiveDan(tcvName, toId);
            return 1;
        }*/
        ...
        //update bảo khố
        if (ADMIN.includes(tcvId) && text.toLowerCase().startsWith('updatebk')) {
	        await fetchItemCh();
	        chat("Done!");
        }
        //update
                ...
                if (args.length > 5 && args[5].toLowerCase() == 'cho' && args[6].toLowerCase() == 'id') {
                    ...
                    axios.get(`https://soi-tcvtool.xyz/truyencv/member/${toId}`).then(
                        response => { 
                            ...
                            if (name !== '' && bp !== '๖ۣۜTiếu ๖ۣۜNgạo ๖ۣۜNhân ๖ۣۜGian') {
                                chat(`Thành viên ID: ${toId} không có trong bang.`);
                                return;
                            }
                            if (name !== '' && bp === '๖ۣۜTiếu ๖ۣۜNgạo ๖ۣۜNhân ๖ۣۜGian') {
