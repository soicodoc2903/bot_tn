    ...
    axios.get(`https://soi-tcvtool.xyz/truyencv/member/${toId}`).then(
        response => { 
            ...
            if (name !== '' && bp !== '๖ۣۜTiếu ๖ۣۜNgạo ๖ۣۜNhân ๖ۣۜGian') {
                chat(`${fromName} - Chuyển ${formatBac(amount)} số dư ([color=red]Không phải bạc[/color]) cho ${name} ([url=https://tutien.net/member/${toId}]${toId}[/url]) - [color=red]Không thuộc bang Tiếu Ngạo[/color] (Y/N)?[br][Phí dịch vụ: ${phiChuyen} bạc][br][color=red]Giao dịch này cần chờ 5s để xác nhận[/color]`);
                setTimeout(function () {
                    setItem(queueKeyRuong, queueDataRuong);
                    setExpire(queueKeyRuong, 15); 
                }, 5000); 
    ...
    */
    ...
    axios.get(`https://soi-tcvtool.xyz/truyencv/member/${toId}`).then(
        response => {
            ...
            if (name !== '' && bp !== '๖ۣۜTiếu ๖ۣۜNgạo ๖ۣۜNhân ๖ۣۜGian') {
                chat(`${fromName} - Chuyển ${formatBac(amount)} bạc cho ${name} ([url=https://tutien.net/member/${toId}]${toId}[/url]) - [color=red]Không thuộc bang Tiếu Ngạo[/color] (Y/N)?[br][Phí dịch vụ: ${phiChuyen} bạc][br][color=red]Giao dịch này cần chờ 5s để xác nhận[/color]`);
                setTimeout(function () {
                    setItem(queueKey, queueData);
                    setExpire(queueKey, 15); 
                }, 5000); 
...
export async function chuyenBacConfirmed(fromId, fromCboxId, toId, amount, total) {
    ...
    if (isSuccess === '1') {
        ...
        chat(`${fromName} - Xong [img]https://cbox.im/i/ETcmb.gif[/img]`);
