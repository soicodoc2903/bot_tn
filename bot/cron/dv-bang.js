...
export async function fetchDuocVien(checkCo = false) {
	...
	let dvStatus = await getItem('dv_status');
	...
	await setItem('dv_status', false);
	dvStatus = false;
        //isAlertResult = false;
    } 

    // Thông báo thu hoạch
    if (dangTrongs.length === 0 && (dvStatus == 'false' || dvStatus == false)) {
        ...
        if (keys.length && (dvStatus == 'false' || dvStatus == false)) {
            chat('Dược viên vừa thu hoạch:' + thuHoachMessage); 
        }
	await setItem('dv_status', true);
	dvStatus = true;
        //isAlertResult = true;
    }
}
